# dev_2 S23 PR63 Alternate-Node Preflight + Conditional SFT Runtime
...
Scope: PM-authorized exactly one fresh bounded alternate-node preflight plus conditional SFT smoke. Eval is not authorized.

## Authorization

```text
PM evidence commit: 5648efd
...
```text
LTP yaml: /tmp/coding-agent-playground-m1-s23-pr63-altnode-preflight-sft-20260521T181207Z.yaml
...
```text
command: ssh -p 31316 -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@10.100.18.14 'set -e; echo HOSTNAME=$(hostname); nvidia-smi --query-gpu=index,name,utilization.gpu,memory.used --format=csv,noheader'
...
Because LTP assigned forbidden node `lg-cmc-b7r202-k07u06-h200-000580`, I stopped/released the allocation before transfer, `/home/xu.yang` capacity probing, mcore import, structured preflight, SFT, or eval. This consumed the one authorized fresh alternate-node attempt.
...
post-stop status command: python3 /work-agents/axrd/workspace/.skill_sources/intern_agent_skills/intern_ltp_skill/scripts/ltp.py status xu.yang~coding-agent-playground-m1-s23-pr63-altnode-preflight-sft-20260521T181207Z
final LTP state: STOPPED (Completed)
final LTP completed: 2026-05-21 18:53:18
final task idx state: STOPPED
endpoint proof: ssh -p 31316 root@10.100.18.14 hostname -> Connection refused
