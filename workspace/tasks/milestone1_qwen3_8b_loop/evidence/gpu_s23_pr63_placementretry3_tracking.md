# dev_2 S23 PR63 Placement Retry 3 GPU Runtime Tracking
...
```text
authorized allocation count: 1 FINAL fresh bounded placement retry
planned LTP frame: xu.yang~coding-agent-playground-m1-s23-pr63-placementretry3-preflight-sft-20260521T190630Z
planned submit yaml: /tmp/coding-agent-playground-m1-s23-pr63-placementretry3-preflight-sft-20260521T190630Z.yaml
forbidden nodes:
...
eval: not authorized
placement mechanism support: unsupported/unknown for physical node exclusion/anti-affinity in current local LTP client/spec
```
...
LTP placement mechanism finding: current local LTP client/spec support for physical-node exclusion/anti-affinity is unsupported/unknown; only VC/sku/image/resource shape were verified
```
...
completed: 2026-05-21 19:13:29
endpoint post-stop: ssh -p 20767 root@10.100.18.14 hostname -> Connection refused
...
Final state: `STOPPED_RELEASED_FINAL_BLOCKED_PLACEMENT_FORBIDDEN_NODE`. Fresh PM authorization is required before any further LTP/GPU/preflight/SFT/eval work.
