...
93. Task-flow rule reaffirmed: PM must organize dev/test work through explicit tasks. Each dev/test PR must cite a task id with owner, acceptance criteria, evidence path, and completion marker before PM gate pass.
94. Owner merge rule reaffirmed: when a dev/test owner self-merges a ready PR, that owner must mark the task complete or blocked-with-final-evidence in task docs or `task_registry.md`, update own `status.md`, update needed history/evidence, and merge/push the completion record.
95. Current PR #18 gate fact: PR body includes task `M1-SFT-SMOKE-DEV4`, owner, acceptance criteria, durable evidence path, and completion marker; latest PM poll shows `mergeable=MERGEABLE` and `mergeStateStatus=CLEAN`, with no required checks reported.
96. PM gate decision: PR #18 passes PM gate for owner self-merge. PM notified dev_4 by tmux inject to self-merge and then write the `M1-SFT-SMOKE-DEV4` completion marker with `mergedAt` and merge commit.
97. PR fact: dev_4 self-merged PR #18 at `2026-05-20T10:18:04Z` with merge commit `1c3a3e23921dd3fc91b340f9b67f83c747d42948`; remaining owner action is to merge/push the completion record marking `M1-SFT-SMOKE-DEV4` blocked-with-final-evidence.
