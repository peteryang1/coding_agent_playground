...
- 2026-05-21 Session 23 SXid different-node runtime authorization:
  - Follow-up evidence classified the blocker as real-or-unknown-time `SXid 20009` / NVLink node-health on `lg-cmc-b7r202-q03u26-h200-000730`, with secondary benign parser noise from `NCCL_ASYNC_ERROR_HANDLING` deprecation warnings.
  - `intern_code_dev_2`: task `M1-S23-SXID-DIFFERENTNODE-PREFLIGHT-SFT-RUNTIME-DEV2`; authorized for exactly one fresh different-node 8xH200 runtime. Avoid `lg-cmc-b7r202-q03u26-h200-000730` and prior failed nodes if selectable. Use `/home/xu.yang/coding_agent_playground/outputs`, no remote source/dependency network, local bundle/data transfer with checksums, structured preflight before SFT, SFT only after PASS plus `sft_allowed=true`, and stop proof. Required evidence: `evidence/dev_2_s23_sxid_differentnode_preflight_sft_runtime.md` and `evidence/gpu_s23_sxid_differentnode_preflight_sft_tracking.md`.
  - `intern_code_dev_4`: task `M1-S23-NCCL-WARNING-PARSER-HYGIENE-DEV4`; create no-execution parser hygiene patch/PR so benign `NCCL_ASYNC_ERROR_HANDLING` deprecation warnings do not count as collective failures when `TORCHRUN_EXIT=0` and `ALLREDUCE_OK`, while preserving real failure detection. Required evidence: `evidence/dev_4_s23_nccl_warning_parser_hygiene.md`.
  - `intern_code_dev_1`: review dev_4 parser-hygiene PR if opened; otherwise stay ready for dev_2 runtime review. Do not run LTP/GPU/SFT/eval.
...
- 2026-05-21 Session 23 PR59 post-merge readiness split:
  - Current gate: PR #59 for `M1-S23-PR57-MCORE-FIX-DEV4` has dev_1/test_1 `PASS_FOR_PM_RETRY` and PM owner-self-merge gate for dev_4. Runtime is still unauthorized until PR #59 is merged/completion-marked and PM issues a separate dev_2 authorization.
  - `intern_code_dev_2`: task `M1-S23-PR59-RUNTIME-READY-DEV2`; prepare no-submit runtime readiness for the expected post-PR59 retry. Evidence path: `evidence/dev_2_s23_pr59_runtime_ready.md`. Include local/provided `mcore_adapter` provenance requirements, file list/checksum plan, exact future transfer command template, destination, post-transfer verification plan, `MCORE_ADAPTER_DIR`, Python import-check command, no-remote-source/dependency-network rule, `/home/xu.yang/coding_agent_playground/outputs`, and stop conditions. Do not submit LTP, run remote commands, transfer, preflight, SFT, eval, or dry-run.
  - `intern_code_dev_1`: task `M1-S23-PR59-RUNTIME-READY-REVIEW-DEV1`; prepare the no-execution review checklist for a future PR59 runtime authorization and record exact blockers if the mcore bundle requirements are underspecified. Evidence path: `evidence/dev_1_s23_pr59_runtime_ready_review.md`. Do not run LTP/GPU/preflight/SFT/eval.
  - `intern_code_test_1`: task `M1-S23-PR59-RUNTIME-READY-GATE-TEST1`; prepare the no-execution test gate for a future PR59 runtime authorization. Evidence path: `evidence/test_1_s23_pr59_runtime_ready_gate.md`. Required checks include local mcore bundle checksum/provenance, post-transfer import proof, structured preflight PASS, `SFT_ALLOWED=true`, checkpoint/model/trainer_state/all_results expectations, and stop/no-running-job proof. Do not run LTP/GPU/preflight/SFT/eval.
